...
public class ScanReceiptJob_INVENTORY extends AbstractJob  {

	public ScanReceiptJob_INVENTORY() {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println(new Date() + "我在执行搜索出境清单回执");
		...
		try {
			...
			List dataList = commonManagerMapper.selectNeedReceiptData(
					"t_inventory", "RETURN_STATUS",
					CommonDefine.NEED_RECEIPT_STATUS_INVERNTORY, true);
					...
					if (receiptGuid != null) {
						for (Object data : dataList) {
							...
							if (receiptGuid.equals(guid)) {
								...
								//回执状态为899情况 更新申报状态 为申报完成
								if (receiptData.get("RETURN_STATUS") != null
										&& Integer.valueOf(receiptData.get(
												"RETURN_STATUS").toString()) == CommonDefine.RETURN_STATUS_899) {
								...
								// 更新数据
								commonManagerMapper.updateInventory(dataMap);
